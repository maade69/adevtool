...
import { downloadAllConfigs, decodeConfigs, fetchUpdateConfig, getVersionsMap, checkUpdates } from '../blobs/carrier'
    ...
    await forEachDevice(
      ...
      async config => {
        ...
        if (config.device.name !== 'tangorpro') {
          ...
          await downloadAllConfigs(updateConfig, outDir, flags.debug, config.device.generation)
